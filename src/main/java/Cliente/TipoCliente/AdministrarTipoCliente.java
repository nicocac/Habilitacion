 ...
 */
public class AdministrarTipoCliente extends JFrame {
...
//        this.setExtendedState(MAXIMIZED_BOTH);
        pack();
        this.setTitle("Consultar Tipo de Cliente");
        inicializaTabla();
...
//            Session session = Conexion.getSessionFactory().openSession()
        ...
        btnEliminar.addActionListener(e -> {
            switch (darBaja()) {
                case 0: {
                    showMessage("Se dio de baja exitosamente el tipo de Cliente.");
                    inicializaTabla();
                    break;
                }
                case 1: {
                ...
                case 2: {
                    showMessage("No se pudo dar de baja el tipo de Cliente.");
                    break;
                }
            }
    ...
    public int darBaja() {
        ...
        try {
            ...
            int i = JOptionPane.showConfirmDialog(null, "Confirma la baja del tipo de Cliente: " + tblTipos.getModel().getValueAt(fila, 1));
            if (i == 0) {
                tx = session.beginTransaction();
                session.update(tipo);
                tx.commit();
//                guardado = txConexion..wasCommitted();
            } else {
                return 1;
            }
        } catch (Exception e) {
